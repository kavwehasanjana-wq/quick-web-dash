...
const PaymentSubmissionsPage = () => {
  ...
  const loadSubmissions = async () => {
    if (loading || loaded || !paymentId) return;
    setLoading(true);
    try {
        ...
        <Card>
          ...
          <CardContent>
            {!loaded ? <div className="text-center py-12">
                ...
                {submissions.map(submission => <Card key={submission.id} className="border border-gray-200 dark:border-gray-700">
                        ...
                        <div className="flex-1 space-y-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="space-y-2">
                              ...
                              {submission.verifiedAt && <div className="flex items-center space-x-2">
                                  <CheckCircle className="h-4 w-4 text-gray-400" />
                                  <span className="text-gray-600 dark:text-gray-400">
                                    <strong>Verified:</strong> {new Date(submission.verifiedAt).toLocaleDateString()}
                                  </span>
                                </div>}
                              {submission.verifierName && <div className="flex items-center space-x-2">
                                  <User className="h-4 w-4 text-gray-400" />
                                  <span className="text-gray-600 dark:text-gray-400">
                          ...
                          {submission.notes && <div className="bg-gray-50 dark:bg-gray-800 p-3 rounded-lg">
                              <p className="text-sm text-gray-600 dark:text-gray-400">
                                <strong>Notes:</strong> {submission.notes}
                              </p>
                            </div>}

                          {submission.rejectionReason && <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg">
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
                            <div className="flex items-center space-x-2">
                              {/* Receipt download */}
                              {submission.receiptFileUrl && <Button variant="outline" size="sm" onClick={() => window.open(submission.receiptFileUrl, '_blank')} className="flex items-center space-x-1">
                                  <Download className="h-3 w-3" />
                                  <span>Download Receipt</span>
                                </Button>}
                              {submission.receiptFileName && <span className="text-xs text-gray-500">
                                  {submission.receiptFileName}
                                </span>}
                            </div>
                            
                            {/* Verify button - prominent placement for InstituteAdmin and Teacher */}
                            {canVerifySubmissions && submission.status === 'PENDING' && (
                              <Button 
                                onClick={() => setVerifyingSubmission(submission)} 
                                className="flex items-center space-x-2 bg-primary hover:bg-primary/90 font-medium shadow-sm"
                                size="sm"
                              >
                            ...
                            {/* Always show verify button for debugging - remove this later */}
                            {canVerifySubmissions && (
                              <div className="text-xs text-gray-500">
                                Status: {submission.status} | Can verify: {canVerifySubmissions.toString()}
                              </div>
                            )}
                          </div>
                        </div>
